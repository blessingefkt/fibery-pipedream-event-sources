...
module.exports = {
    ...
    methods: {
        getClient() {
            const host = get(this, "$auth.account_name");
            const token = get(this, "$auth.api_key");
            if (!host || !token) {
                throw new Error('Invalid auth object.');
            }
            return new Fibery({
                host: host.trim() + '.fibery.io',
                token: token.trim()
            });
        },
        ...
        async appendSelects(type, fieldNames) {
            const typeFields = type['fibery/fields'];
            const selects = this.getDefaultFieldNames(typeFields);
            await Promise.all(typeFields
                ...
                .map(async fibField => {
                    ...
                    const fieldType = await this.getType(fieldTypeName);
                    ...
                    if (isPrimitive) {
                        ...
                        if (isRelation) {
                            fields.push(...this.getDefaultFieldNames(fieldType['fibery/fields']));
                        }
                        if (isComplexType) {
